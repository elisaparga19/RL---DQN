...
class Dreamer():
    ...
    def replace_target_network(self):
        with torch.no_grad():
            for param, target_param in zip(self.value_model.parameters(), 
                                           self.target_value_model.parameters()):
    ...
    def _compute_loss_world(self, state, data):
       '''Computes the loss between the the predicted state and the received data'''
    ...
    def update_parameters(self, data, gradient_steps):
        ...
        for s in tqdm(range(gradient_steps)): # collect interval
...
def cal_returns(reward, value, bootstrap, gamma, lambda_):
  """
  ...
  inputs = reward + gamma * next_value * (1 - lambda_)  # dim=[horizon, (chuck-1)*B, 1]
  outputs = []
  last = bootstrap

  for t in reversed(range(reward.shape[0])): # for t in horizon
    inp = inputs[t]
    last = inp + gamma[t] * lambda_ * last
    outputs.append(last)
