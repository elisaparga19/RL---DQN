...
def train_agent(env, agent, expert_agent, replay_buffer, rollout_episodes, training_iters, filename):

    tr_iters_vec, avg_rew_vec, std_rew_vec, sr_vec = [], [], [], []
    _, (axes) = plt.subplots(1, 2, figsize=(12,4))
    
    demonstrations, _, _, _ = perform_rollouts(env, expert_agent, rollout_episodes, replay_buffer, False)
    ...
    save_metrics(tr_iters_vec, avg_rew_vec, std_rew_vec, sr_vec, filename)
...
def save_metrics(tr_iters_vec, avg_reward_vec, std_reward_vec, sr_vec, filename):
...
if __name__ == '__main__':
    ...
    if env_name == 'CartPole-v0':
        ...
        # Complete using P1
        expert_agent = PIDController(0.1, 0.01, 0.01, 0.02)
    ...
    train_agent(env=env, agent=agent, expert_agent=expert_agent, 
                replay_buffer=replay_buffer, rollout_episodes=nb_rollouts, training_iters=20000, filename=filename)
