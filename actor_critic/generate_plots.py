...
exp_path = os.path.abspath('../actor_critic')
...
if __name__ == '__main__':
    ...
    choice = int(input('To select Pendulum enter 0, to select CartPole enter 1, to select Acrobot enter 2: '))
    if choice == 0:
        for extra_str in ['', '_critic']:
            i = 2
            for j in range(1,4):
                path_exp_csv = exp_path + '\pendulum_exp_' + str(j) + str(i) + extra_str
                filename = 'pendulum_exp_' + str(j) + str(i) +  extra_str + '.pdf'
                df_list = []
                for k in range(1, 4):
                    df = pd.read_csv(path_exp_csv + '\pendulum_exp_' + str(j) + str(i) + extra_str + '_' + str(k) + '.csv', sep='\t')
        ...
        for i in range(1,3):
            for j in range(1,4):
                path_exp_csv = exp_path + '/acrobot_exp_' + str(j) + str(i)
                filename = 'acrobot_exp_' + str(j) + str(i) + '.pdf'
                df_list = []
                for k in range(1, 4):
                    df = pd.read_csv(path_exp_csv + '/acrobot_exp_' + str(j) + str(i) + '_' + str(k) + '.csv', sep='\t')
                    df = df.rename(columns={'avg_reward':'avg_reward_' + str(k), 'std_reward':'std_reward_' + str(k)})
                    df_list.append(df)
                full_df = pd.concat(df_list, axis=1)
                full_df['avg'] = full_df[['avg_reward_1', 'avg_reward_2', 'avg_reward_3']].mean(axis=1)
